...
int tryVmaMap(uint64 va) {
  ...
  if(v->prot & PROT_EXEC)
    flags |= PTE_X;
  if(mappages(p->pagetable, va, PGSIZE, (uint64)pa, flags) < 0) {//if(mappages(p->pagetable, va, PGSIZE, (uint64)pa, PTE_R | PTE_W | PTE_U) < 0) {
    panic("tryVmaMap(): mappages failed!");
  }
  return 1;
}
